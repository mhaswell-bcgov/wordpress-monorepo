...
import { PanelBody, ButtonGroup, Button, ToggleControl, Notice } from '@wordpress/components';
...
export default function Edit({ attributes, setAttributes }) {
	 ...
	 */
	const exampleHierarchy = useMemo(() => ['Grandparent', 'Parent', 'Child'], []);
		...
		<>
			...
			<InspectorControls>
				<PanelBody title={__('Breadcrumb Settings')}>
					<Notice className="dswp-block-setting-warning" status="warning" isDismissible={false}>
					{__('This block is limited to page hierarchies. Post type support upcoming.')}
					</Notice>
					{/* Divider Type Selector */}
					<div className="dswp-divider-selector">
						<p className="dswp-block-setting-label">{__('Divider Type')}</p>
						<ButtonGroup>
							{['slash', 'chevron'].map(type => (
								<Button
									key={type}
									variant={dividerType === type ? 'primary' : 'secondary'}
									onClick={() => setAttributes({ dividerType: type })}
									isPressed={dividerType === type}
								>
									<span
										className={`dashicons ${type === 'slash'
