...
const useDocumentManagement = ({
     ...
     */
    const handleBulkDelete = useCallback(async (selectedDocuments) => {
        ...
        try {
            ...
            if (onShowNotification) {
                onShowNotification('success', __('Selected documents were deleted successfully.', 'bcgov-design-system'));
            }
        } catch (error) {
            if (onError) {
     ...
     */
    const handleSingleDelete = useCallback(async (documentId) => {
        try {
            await onDelete(documentId);
            setDeleteDocument(null);
            
            if (onShowNotification) {
                onShowNotification('success', __('Document deleted successfully.', 'bcgov-design-system'));
            }
        } catch (error) {
            if (onError) {
                onError('delete', documentId, error, {
                    customMessage: sprintf(
                        __('Error deleting document %d: %s', 'bcgov-design-system'),
                        documentId,
                        error.message || __('An unknown error occurred', 'bcgov-design-system')
                    )
                });
            }
        }
