 ...
 * @returns {Object} Metadata management state and functions
 */
const useMetadataManagement = ({ 
    ...
    }, [metadataState.editingMetadata, metadataState.editedValues, metadataFields]);
    ...
    const handleMetadataChange = useCallback((documentId, fieldId, value) => {
        ...
        const hasChanges = Object.entries(newBulkMetadata).some(([docId, editedMetadata]) => {
        ...
        try {
            ...
            setLocalDocuments(prev => prev.map(doc => 
                doc.id === editingMetadata.id 
                    ? { ...doc, metadata: { ...doc.metadata, ...editedValues } }
                    : doc
            ));
            ...
            if (onShowNotification) {
                onShowNotification('success', __('Document metadata updated successfully', 'bcgov-design-system'));
            }
        } catch (error) {
    ...
    }, [metadataState.editingMetadata, metadataState.editedValues, apiNamespace, localDocuments, onUpdateDocuments, onShowNotification, onError]);

    /**
     * Toggle spreadsheet mode on/off
     * @param {boolean} enabled Whether to enable spreadsheet mode
     */
    const toggleSpreadsheetMode = useCallback((enabled) => {
        if (enabled) {
    ...
    const handleSaveBulkChanges = useCallback(async () => {
        ...
        try {
            ...
            if (failed.length > 0) {
                ...
                if (onShowNotification) {
                    onShowNotification('success', __('All metadata changes saved successfully.', 'bcgov-design-system'));
                }
    ...
    }, [metadataState.bulkEditedMetadata, apiNamespace, onError, onShowNotification]);
