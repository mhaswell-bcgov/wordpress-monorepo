...
const VirtualDocumentList = ({
  ...
  const isDocumentSelected = useCallback((document) => {
    if (!selectedDocuments || !document) return false;
    const documentId = document.id || document.ID;
    return selectedDocuments.some(d => (d.id || d.ID) === documentId);
  }, [selectedDocuments]);
  
  // Render a single document row
  const renderDocument = useCallback(({ item: document, index, style }) => {
    ...
    return (
      <div 
        className={`virtual-list__item ${isSelected ? 'selected' : ''}`} 
        style={style}
      >
        <div className="document-row">
          {showCheckboxes && (
            <div className="document-row__checkbox">
              <input 
                ...
                aria-label={`Select ${document.title || document.post_title}`}
              />
              <label htmlFor={`document-${documentId}`} className="visually-hidden">
                Select {document.title || document.post_title}
              </label>
            </div>
          )}
          ...
          <div className="document-row__meta">
            <span className="document-row__date">
              {document.date || document.post_date || 'No date'}
            </span>
          </div>
            ...
            {renderActions ? (
              ...
              <>
                <button 
                  ...
                  aria-label={`Edit ${document.title || document.post_title}`}
                >
                  Edit
                </button>
                <button 
                  ...
                  aria-label={`Delete ${document.title || document.post_title}`}
                >
                  Delete
                </button>
              </>
