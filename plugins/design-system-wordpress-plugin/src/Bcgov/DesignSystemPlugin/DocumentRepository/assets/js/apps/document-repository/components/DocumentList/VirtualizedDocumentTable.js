            ...
            <table className="document-table">
                <thead className="document-table__header">
                    <tr>
                        ...
                        <th className="actions-column">{__('Actions', 'bcgov-design-system')}</th>
                    </tr>
                </thead>
                <tbody style={{ height: `${totalHeight}px`, position: 'relative' }}>
                    {documents.length === 0 ? (
                        <tr>
                            <td colSpan={metadataFields.length + 4} className="empty-state">
                                {__('No documents found.', 'bcgov-design-system')}
                            </td>
                        </tr>
                    ) : (
                        <div className="virtualized-rows" style={{ 
                            position: 'absolute', 
                            top: `${topOffset}px`, 
                            width: '100%'
                        }}>
                            {visibleDocuments.map(document => {
                                ...
                                const rowClass = isSelected ? 'document-table__row document-table__row--selected' : 'document-table__row';
                                ...
                                return (
                                    <tr key={document.id} className={rowClass}>
                                        ...
                                        <td>
                                            <div className="document-cell">
                                                <span className="document-cell__title">
                                                    {document.title || document.filename || __('Untitled', 'bcgov-design-system')}
                                                </span>
                                                <span className="document-cell__metadata">
                                                    {document.filename || document.title || __('No filename', 'bcgov-design-system')}
                                                </span>
                                            </div>
                                        </td>
                                        ...
                                        {metadataFields.map(field => {
                                            const fieldValue = document.metadata?.[field.id] || '';
                                            const cellValue = isSpreadsheetMode ? (
                                                bulkEditedMetadata[document.id]?.[field.id] ?? fieldValue
                                            ) : fieldValue;
                                            
                                            if (isSpreadsheetMode) {
                                                return (
                                                    <td key={field.id} className="editable-cell">
                                                        <input
                                                            type="text"
                                                            className="editable-field"
                                                            value={cellValue}
                                                            onChange={e => onMetadataChange(document.id, field.id, e.target.value)}
                                                        />
                                                    </td>
                                                );
                                            }
                                            
                                            return <td key={field.id}>{cellValue}</td>;
                                        })}
                                        
                                        <td className="file-size">
                                            {document.metadata?.document_file_size ? 
                                                formatFileSize(parseInt(document.metadata.document_file_size)) : 
                                                __('N/A', 'bcgov-design-system')}
                                        </td>
                                        <td className="actions-column">
                                            <div className="action-buttons">
                                                <button
                                                    ...
                                                    title={__('Edit Document', 'bcgov-design-system')}
                                                >
                                                ...
                                                {document.metadata?.document_file_url && (
                                                    <a
                                                        ...
                                                        title={__('View Document', 'bcgov-design-system')}
                                                    >
                                                        <svg viewBox="0 0 24 24" width="24" height="24">
                                                            <path
                                                                fill="currentColor"
